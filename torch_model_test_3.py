"""
此文件用于实现5折交叉验证
输入的数据文件夹为 root下有两个:negative和positive（此处需要注意）
最后绘制的是ROC曲线以及要计算AUC
因此，最后绘制的是一幅图片中有五个ROC曲线的，并且还有AUC的图示
比较离谱的是，现在只能用5折，k改成其他值的话，会报错，导致auc计算为nan
后来发现，是样本数据分布不平均的问题，尤其是positive的数量比negative少太多了，可以尝试调大bath_size
现在的任务是，搞定混淆矩阵以及tpr有时候为nan的问题，以及绘制出来的roc曲线非常乱，没法看
现在是，调大了bath_size，以及去掉了最后的不够一个bath_size的数据，基本不会出现nan的现象了
现阶段的目标是，解决预测得分为负值的问题，这个也是绘制出来的roc曲线有问题的原因
后来查明，预测得分为负值没有关系
还有就是混淆矩阵标签的问题，这个classes变量一直是一个缺失的状态
"""
# PaddyDataSet
import torch
import random
...
filepath = "D:/学习/大创/data/训练数据集/data/melspec(1000_50)"
# 权重文件位置
...
# （0）参数设置
# -------------------------------------------------- #
batch_size = 128  # 每个step训练batch_size张图片
...
for train_index, val_index in kf.split(data_0):
    ...
    plt.savefig("D:/学习/大创/data/训练数据集/model/photo/" + dir_path + "/model_loss_第{}折_".format(k) + str(nowTime) + ".jpg")
    # plt.show()
    # plt.xlim((0,50))
    # plt.ylim((0,1))
    plt.figure()
    ...
    plt.savefig("D:/学习/大创/data/训练数据集/model/photo/" + dir_path + "/model_acc_第{}折_".format(k) + str(nowTime) + ".jpg")
...
data_x = np.array(avg_x)
data_y = np.array(avg_y)
avg = sum / k
print(data_x)
print(data_y)
plt.plot(np.mean(data_x, axis=0), np.mean(data_y, axis=0), label='AVG (auc = {0:.2f})'.format(avg), c=clr_2, alpha=1, linewidth=2)
