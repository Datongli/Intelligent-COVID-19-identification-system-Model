...
# 数据集文件夹位置
filepath = r"F:\Database\Track1+CoughVid(增强)(mel)(cough_decated)"
# 权重文件位置
...
# 写一个txt文件用于保存超参数
file = open(photo_folder + '\\' + str(net_num) + '层网络 ' + nowTime + ".txt", 'w',encoding='utf-8')
        ...
        with torch.no_grad():  # 下面不进行梯度计算
            ...
            if val_loss[-1] >= min_val_loss + 0.1:
                if (epoch + 1) - epoch_num >= 4:
                    # 保存的权重名称
                    savename = work_path + '/model_' + dir_path + "_第{}折验证{}层网络".format(k_num, net_num) + '.pth'
                    # 保存当前权重
            ...
            if acc_val > best_acc_all:
                ...
                # 保存的权重名称
                savename = work_path + '/model_' + dir_path + '最好的权重{}层网络'.format(net_num) + '.pth'
                # 保存当前权重
                torch.save(net.state_dict(), savename)
